...
一句话看进度：**8 份资料的 3,977 页已有页级处理基线；全局索引目前只纳入 S005 的部分内容和 S008，尚未完成八源全量入库。** 以下数量采用 2026-08-21 的已核验快照，不把进行中的修改算成完成。
...
| ID | 资料 | 页数 | 当前阶段 |
|---|---|---:|---|
| S001 | 《化工原理（上）》 | 371 | 页级基线完成；附录与答案补充权威已独审通过；正式 1.1 投影进行中 |
| S002 | 《化工原理（下）》 | 316 | 页级基线完成；包外答案/参考页处置已独审通过；正式 1.1 投影进行中 |
...
| S005 | 《化工数据》 | 427 | C02–C12/附录已进入阶段性全局索引；C01 内容初稿已完成自查、待独立复核；前后置页与参考文献关联处理中 |
| S006 | 《化工计算与软件应用（第二版）》 | 547 | 9 个物理包和 815 个视觉对象清单完成；解释、UI 复核与边界修复未完成 |
| S007 | 《化工设备设计全书·塔设备》 | 398 | 674 个对象分三批修复；第 1 批通过，第 2 批独审发现 138 个对象需修复、修复中，第 3 批待处理 |
