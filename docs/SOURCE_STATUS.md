...
### 本地流体流动章来源层更新（2026-09-16）

物理页 **18–29，共12页、232个来源内容元素**，已完成完整页转写、非作者来源复核、确认差异修订和主审限定接纳。元素用于保存标题、自然段、原式与图的阅读顺序，不是232个检索条目。原例题中体积流量基准未明确写出的疑点照录保留；不把原题静默改写成标准体积流量，也不将教材数值用作项目默认。

当前只接纳上述12页。物理页30–38仍在重做，第39–46页处于独立复核；不代表整个流体流动章或全书通过。这轮仍为私有来源层，新增公开KU与向量均为0，既有98KU、113行真实语义向量未改变。

两道实际hybrid试查已召回相关主题，但尚未完整给出轴功率—效率关系、平均—最大速度关系及限定幂律的条件。拟在既有条目内最小补强，不另拆细碎知识；新句、来源证明和对应向量尚未更新。这是具体使用问题暴露的内容覆盖缺口，不是新的正式评测结果。
