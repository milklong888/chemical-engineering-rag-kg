...
### 本地流体流动章来源层更新（2026-09-17）

物理页 **18–46，共29页、518个来源内容元素**，已完成完整页转写、非作者来源复核、确认差异修订和主审限定接纳，并建立本地统一阅读入口。元素用于保存标题、自然段、原式与图的顺序，不是518个检索条目。原例题中体积流量基准未明确写出的疑点照录保留；不把原题静默改写成标准体积流量，也不将教材数值用作项目默认。

本地入口串联例题、续句和方程，不重复复制跨段正文；总压头与测压液面、例题换基准面等容易误读的地方已对照原页确认。第46页的量纲分析仍续至第47页，后续页不在本批验收范围；这不代表整个流体流动章或全书通过。来源转写本身不进入公开检索，既有98KU、113行真实语义向量尚未改变。

两道实际hybrid试查暴露了轴功率—效率关系、平均—最大速度关系及限定幂律条件的覆盖缺口。对应两条原创补充已通过限定来源复核和主审，复用既有A3/A4，不新增细碎主题；其公开数据、修订证明和真实向量正在隔离暂存区同步构建，尚未作为新版本发布。试查不是新的正式评测或普遍准确性证明。
