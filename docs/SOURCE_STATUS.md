...
本页描述独立项目 `chemical-engineering-rag-kg` 的选题知识预览，以及与之明确分开的旧八源历史基线。

## 当前统一检索范围

[统一查询清单](../knowledge/curated-collection-v1.json)只包含四源包和 S001 上册选题包，合计 5 个来源卷、98 KU、113 行 512 维真实语义向量。统一入口为 `src/query_collection.py`，复用原包载荷，不重建或混入旧八源阶段索引。两个包的图谱仍分别保存，跨包关联用于可追溯补读，不将重复主题根计成新的学科。

## 新增 S001 上册选题包

[`curated-s001-upper-v1`](../knowledge/curated-s001-upper-v1/) 保存 S001《化工原理 上》的 29 个已审原创释义。来源为王瑶、贺高红主编，化学工业出版社，2016 年 10 月第 1 版第 1 次印刷，ISBN 978-7-122-27413-7；实际 PDF 为 371 页、83,845,722 字节，身份见包内来源清单。`CEPR-UPPER-WANG-HE` 和 `core_upper` 是同一来源的旧别名，不重复登记成新书。
...
本批包含 64 个节点、63 条内部边、29 条来源定位证据、17 条跨包关联及 95 条外部 Skills 关联。真实 BGE 向量为 31 行、512 维；B9 与 C6 在索引层各分两段，全部 KU 正文完整保留。所选主题通过不代表第1章、附录完整表体、习题答案或全部原始公式图表已校订。

旧 S001 的 11 包结构候选与局部修复继续独立保留。核查冻结输入发现，后来的蒸发章三项闭环与 p369 答案修复未自动写回旧投影；不能据此把旧全书候选标为已准入。本次 29 KU 使用单独审核、单独来源链，不覆盖旧候选，也不与历史片段数相加。

原始 PDF 与源页仅保留本地；`full_source_digitization_complete=false`、`human_review=false`、`production_activated=false`。

后续本地核查：绪论 p12–17 已按完整页图评估用途，历史/分类导读、基础总纲和单位算例无独立信息增量，本轮不另建 KU。另新增并核对六页私有校订正文层，保留表 1.1 的源页空白格和跨页单位换算；旧机器正文与旧投影未覆盖。这是限定正文层的代理复核，不是整章所有图表对象或全书数字化验收。附录 p332 表 2 的 15 行源转写已完成限定核对，保留四处原印单位/量纲疑点并禁止据疑点自动计算；旧裁图漏掉末行，不能沿用其像素校验作为整表覆盖证明。两项补充都只保留私有来源层，不公开表体、校订全文或新增检索条目。

### 本地附录来源层更新（2026-09-16）

附录物理页 **332–366，共 35 页、68 个来源对象**，已完成分段转写、独立来源复核、确认差异修订和主审接纳。覆盖单位与物性表、共线图及配套表、设备性能与规格表、尺寸图和结构代号。本轮关闭 16 处确认的转写字段差异，原印异常值仍保留，不按常识改成“正确参数”。

本地统一入口按原页顺序引用三段已审内容，保留 10 组真实续表和 7 组图—配套表关系，不重复复制表体。物理页 347 的一个低清物质名、物理页 362 的部分重叠型号标签仍明确标为未读清；这不是所有字符均已解决或当前工程参数已获验证。

这批只补齐私有来源层，**新增公开 KU 为 0、向量为 0**：历史参数、单个物质名和型号不逐项拆成知识节点，已有完整方法继续复用。公开检索仍为 98 个主题、113 行真实语义向量。原始 PDF、页图及完整转写不公开；旧全书投影尚未重建，整本资料的数字化仍未完成。
