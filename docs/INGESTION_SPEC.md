...
已审原创释义可以按明确选题范围先入库，使用 [选题知识合同](../contracts/curated_knowledge_contract.json)。验收单位是一块能完整解释问题及必要条件的知识，不是一个符号或一个句子。选题包保留来源身份、真实页码、章节或明示的主题支持组、粗知识单元与证据关系；不编造未处理页的页账本，也不因此宣称整本书数字化完成。

已有其他项目的已审内容可以复用，但必须重新核对来源和正文身份，并完成本项目的关系、资格和公开检查。另一项目发布成功不算本项目已入库。

新增选题包使用独立 `bundle_id`，`source_chain_id` 必须精确等于 `{bundle_id}:{source_id}`，不能冒用首批身份。来源别名以原件哈希归并。同项目旧批次已完整覆盖的知识优先引用，不复制正文；跨批关联单独存为 `cross_bundle_references.jsonl`，包含起点 KU、目标批次、目标 KU 和目标正文哈希。外部 Skills 关联仍存入其独立命名空间。新包须有自身内容、图谱、向量与发布校验，不能只扩大旧包的数量检查。

一个知识块超出模型窗口时，仅为索引作无损分段：各段保留标题、适用条件及原文字符范围，合计完整覆盖原文；命中后返回完整知识块。不得截掉后半段、改写成更短摘要，或把技术分段当作多个新知识单元。

源文件 SHA 与页码构成 `source_document_locator` 证据，不等于页图或裁片的字节哈希。私有原件和审阅记录留存供合法复核，公开包不包含原始 PDF、页图、OCR 全文或私人路径。对明确获准的原创释义，可发布经过验证的派生向量预览；模型、数据库及旧未获准的源文向量仍不公开。公开预览不等于生产检索激活，代理审阅不写作人类逐条审核。
