...
## 0. 先判断实际用途，再决定是否做 RAG

每个检索候选先说明“能帮助解决什么实际问题”。不以页数、对象数、填满字段或重述所有文字为目标。按信息本身的用途分类，而不是看它属于正文、公式还是截图：

- **检索候选**：可用于计算、设计或选型、故障诊断，或准确解释关键概念、适用边界与风险；通过来源和内容验收后再决定检索资格。
- **仅关联支撑**：数据库/模型选择、参考态、局部操作条件等信息对理解主内容必要，但不能独立回答问题；挂到对应知识单元，不独立检索或嵌入。
- **不做 RAG**：纯导航/欢迎步骤、装饰性界面、已经完整表达的重复内容、无独立语义的文字碎片及其他无信息增量的材料；不生成独立 RAG chunk、embedding 或粗知识单元。必要时只保留页码、来源或重复关系。

来源页留档和完整表体的数字化不意味着每页、每格都应成为检索条目。解释长度以实际使用所需为限；无价值内容不为了“全量”而扩写。反过来，有价值但存在源疑点的内容应保留问题和证据，不能只因难处理就判为无价值，也不能在疑点未核实前当作可靠答案。
...
不能只因外观像设置页就丢弃：如果其中的数据库、参考态、计算模型或参数选择是理解结果所必需且正文未重复说明的信息，应保留相应证据和简短解释；只起支撑作用的标为 `supporting_content`，不作为独立答案开放。仅转写实际可见字段，不补造隐藏结果，并分别注明“已阅读截图”和“已实际运行复现”，二者不能互相替代。
